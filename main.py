...
class DomainWindow(TabbedPanel, Screen):
    ...
    def Attack(self, name):
        ...
        for t in DomainWindow.domains:
            ...
            try:
                session = HTMLSession()
                r = session.get(f'https://webiplookup.com/{target}/domain.htm')
                ss = r.html.links
            except:
            ...
            try:
                for i in ss:
                        if target in i and "homepagecached" not in i:
                            rs = i.replace("/", "").replace("https:", "")
                            subdomains.append(rs)
                        else:
                            pass
            except:
